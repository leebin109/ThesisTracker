...
const Cell = ({ label, children, accent, style }) => (
  <div style={{ border: `1px solid ${T.border}`, background: T.surface, position: 'relative', ...style }}>
    {label && (
      <div style={{
...
const CommandBar = ({ symbol, onSymbol, activePanel = 'F1', onSearch, onSettings, refreshing, panelDefs, providerStatus, alertCount = 0, onAlerts }) => {
  ...
  return (
    <div style={{
      ...
      <div className="cmd-fkeys" style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 10, flex: '0 0 auto' }}>
        {fkeys.map(f => {
...
const HeroStrip = ({ stock, onRefresh, refreshing }) => {
  ...
  const heroNameSize = heroName.length > 34 ? 16 : heroName.length > 24 ? 18 : heroName.length > 16 ? 20 : 22;
  return (
    <div style={{
      display: 'grid',
      gridTemplateColumns: 'auto minmax(320px, 1.45fr) 1fr 1fr 1.2fr 0.9fr 1fr',
      gap: 0, padding: '14px 18px',
      background: `linear-gradient(180deg, ${T.surface} 0%, ${T.bg} 100%)`,
      borderBottom: `1px solid ${T.border}`, alignItems: 'center',
    }}>
      <ScoreRing score={stock.scores?.overall ?? 0} size={68} accent={T.amber}/>
      <div style={{ paddingLeft: 16, paddingRight: 16, borderRight: `1px solid ${T.borderSoft}`, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: T.inkDim, fontWeight: 600, letterSpacing: '0.04em', minWidth: 0 }}>
          <span style={{ whiteSpace: 'nowrap' }}>{stock.symbol} · {stock.market} · {stock.currency}</span>
          <span style={{ fontSize: 14, flex: '0 0 auto' }}>{stock.flag || ''}</span>
          ...
          {onRefresh && (
            <button onClick={() => onRefresh(stock.id)} disabled={refreshing}
              style={{ marginLeft: 6, padding: '2px 8px', fontSize: 9, fontFamily: T.font,
...
const PitchHeadline = ({ text, onEdit }) => (
  <div style={{
    padding: '14px 22px', background: T.bg, borderBottom: `1px solid ${T.border}`,
    display: 'flex', alignItems: 'center', gap: 14,
  }}>
    <div style={{ fontSize: 9, color: T.amber, letterSpacing: '0.18em', fontWeight: 700,
      writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>PITCH</div>
    <div style={{ width: 2, alignSelf: 'stretch', background: T.amber, boxShadow: `0 0 8px ${T.amber}88` }}/>
    <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 19, fontWeight: 500, color: T.ink,
      lineHeight: 1.4, letterSpacing: '-0.01em', flex: 1 }}>
      "{text || '—'}"
    </div>
    {onEdit && (
