    ...
    sections: [
      {
        ...
        paragraphs: [
          ...
          '특정 종목의 매수·매도 추천이나 투자자문을 제공하지 않으며, 모든 투자 판단과 그 결과에 대한 책임은 사용자 본인에게 있습니다.', // audit-copy-ignore: 부정형 면책 문장
        ],
      },
      {
        heading: '데이터와 점수의 한계',
...
function MacroPanel({ stock, stocks, watchlistIds, apiSettings }) {
  ...
  const fetchAll = async () => {
    ...
    } catch (e) { setError(isPersonalOnlyError?.(e) ? `POLICY: ${e.message}` : (e.message || '데이터 로드 실패')); }
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 10, overflowY: 'auto' }}>
      <Cell label={`MACRO CORRELATION · ${stock.symbol} vs 글로벌 매크로 (1Y 주간 수익률 기준)`} accent={T.cyan}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          {error && <span style={{ color: error.startsWith('POLICY:') ? T.yellow : T.red, fontSize: 10 }}>{error}</span>}
...
function FinancialHistorySection({ stock, apiSettings, dartCorpMap, onSaveHistory }) {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      setError(isPersonalOnlyError?.(e) ? `POLICY: ${e.message}` : (e.message || '수집 실패'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <Cell label="5Y FINANCIAL HISTORY" accent={T.amber}>
      <div style={{ padding: '8px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {error && <span style={{ fontSize: 10, color: error.startsWith('POLICY:') ? T.yellow : T.red }}>{error}</span>}
...
function ChartPanel({ stock, apiSettings, onRefresh, refreshing, fetchStatus, period: periodProp, chartType: chartTypeProp, onPeriodChange, onChartTypeChange }) {
  ...
  useEffect(() => {
    ...
    fetchYahooChartOhlc(sym, periodDef.range, periodDef.interval, apiSettings)
      ...
      .catch(e => { if (!cancelled) { setChartErr(isPersonalOnlyError?.(e) ? `POLICY: ${e.message}` : e.message); setChartLoading(false); } });
  ...
  return (
    <Cell label={`PRICE CHART · ${period}`} accent={T.amber} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
        ...
        {chartErr && <span style={{ color: chartErr.startsWith('POLICY:') ? T.yellow : T.red, fontSize: 9.5 }} title={chartErr}>{chartErr.startsWith('POLICY:') ? 'POLICY' : 'ERR'}</span>}
      </div>
        ...
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 12, height: '100%', overflowY: 'auto', fontSize: 11 }}>
          ...
          {alertErrors && alertErrors.length > 0 && (
            <div style={{ fontSize: 10, color: T.yellow, lineHeight: 1.5, borderTop: `1px solid ${T.borderSoft}`, paddingTop: 8 }}>
              <div style={{ color: T.inkFaint, fontSize: 9, letterSpacing: '0.12em', marginBottom: 4 }}>
                {alertErrors.every(err => err.personalOnly) ? 'POLICY STATUS' : 'RECENT ERRORS'}
              </div>
