...
한국어로 간결하고 정확하게 답변하세요.`;
  ...
  // localhost → 로컬 프록시 경유 (CORS 우회), 외부 URL → 직접 접근
  const ollamaBase = () => isProxiedOrigin() ? '' : ollamaUrl.replace(/\/$/, '');
  const ollamaPath = (path) => isProxiedOrigin() ? `/api/ollama${path}` : `${ollamaUrl.replace(/\/$/, '')}${path}`;
