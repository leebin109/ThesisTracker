...
function DcfMiniModeler({ stock }) {
  ...
  // Derive shares outstanding (M) from DART EPS or implied via PER+price
  const autoShares = (() => {
    const h = history[0];
    if (!h) return null;
    const net = h.netIncome;
    if (!Number.isFinite(net) || net <= 0) return null;
    // DART provides eps in 원 for KRW stocks; netIncome in 억원
    if (isKrw && Number.isFinite(h.eps) && h.eps > 0) {
      const s = Math.round(net * 100 / h.eps);
      if (s > 0) return s;
    }
    // Fallback: implied EPS from PER and current price
    const per = Number(stock.metrics?.per);
    const price = Number(stock.price);
    if (Number.isFinite(per) && per > 0 && Number.isFinite(price) && price > 0) {
      const epsD = price / per;
      const s = isKrw
        ? Math.round(net * 100 / epsD)  // 억원 * 1e8 / 원 / 1e6 = 억원*100/원
        : Math.round(net / epsD);        // M USD / USD = M shares
      if (s > 0) return s;
    }
    return null;
  })();
