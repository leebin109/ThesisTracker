...
function summarizeApiPayloadText(text) {
  ...
  if (!raw) return '';
  if (raw.startsWith('<')) return raw.replace(/\s+/g, ' ').slice(0, 180);
  try {
    ...
    if (data.upstreamBody) parts.push(String(data.upstreamBody));
    const yahooErr = data?.chart?.error || data?.quoteSummary?.error || data?.quoteResponse?.error || data?.finance?.error || data?.timeseries?.error;
    if (yahooErr) parts.push(yahooErr.description || yahooErr.message || yahooErr.code || JSON.stringify(yahooErr));
...
function detectJsonApiError(data, opts = {}) {
  const openDartOkStatuses = opts.openDartOkStatuses || ['000'];
  if (data?.status && !openDartOkStatuses.includes(String(data.status))) {
    return `status ${data.status}${data.message ? ` · ${data.message}` : ''}`;
  }
  const yahooErr = data?.chart?.error || data?.quoteSummary?.error || data?.quoteResponse?.error || data?.finance?.error || data?.timeseries?.error;
  if (yahooErr) return yahooErr.description || yahooErr.message || yahooErr.code || JSON.stringify(yahooErr);
  if (data?.error && !opts.allowErrorBody) {
...
async function fetchJsonWithDiagnostics(label, url, opts = {}) {
  ...
  const summary = summarizeApiPayloadText(text);
  if (!res.ok) {
    throw new Error(`${label} HTTP ${res.status}${summary ? ` · ${summary}` : ''}`);
  }
  if (text.trimStart().startsWith('<')) {
    throw new Error(`${label} HTML response${summary ? ` · ${summary}` : ''}`);
  }
  let data;
  try {
    data = text ? JSON.parse(text) : null;
  } catch {
    throw new Error(`${label} JSON parse failed${summary ? ` · ${summary}` : ''}`);
  }
  const apiErr = detectJsonApiError(data, opts);
  if (apiErr) throw new Error(`${label} ${apiErr}`);
  return data;
}
...
async function fetchYahooEarnings(symbol) {
  ...
  const url = isProxiedOrigin()
    ...
    : `https://query2.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(symbol)}?${params}`;
  try {
    const data = await fetchJsonWithDiagnostics('Yahoo earnings', url);
...
function buildOpenDartApiUrl(endpoint, params) {
  ...
  return isProxiedOrigin()
    ? `/api/proxy?service=opendart&path=${encodeURIComponent(endpoint)}&${q}`
...
function buildYahooChartUrl(symbol, params) {
  ...
  return isProxiedOrigin()
    ? `/api/proxy?service=yahoo&path=chart&symbol=${encoded}&${q}`
...
function buildYahooSearchUrl(params) {
  const q = params instanceof URLSearchParams ? params.toString() : new URLSearchParams(params).toString();
  return isProxiedOrigin()
    ? `/api/proxy?service=yahoo&path=search&${q}`
...
async function fetchOpenDartStatementsLegacy(corpCode, apiSettings) {
...
async function fetchOpenDartStatementsReliable(corpCode, apiSettings) {
  const { openDartKey: key, dartFiscalYear: year, dartReportCode: reportCode, dartFsDiv } = apiSettings;
  const years = [...new Set([
    Number(year),
    Number(year) - 1,
  ...
  throw new Error(`OpenDART statements unavailable · ${errors.join(' / ')}`);
}
...
async function fetchStockData(stock, apiSettings, cache, dartCorpMap, onStatus = () => {}) {
    ...
    if (apiSettings.openDartKey) {
      ...
      if (dartCache && (dartCache.schemaVersion ?? 1) >= CACHE_SCHEMA_VERSION) {
        ...
        const [rawRes, sharesRes] = await Promise.allSettled([
          fetchOpenDartStatementsReliable(corp.corpCode, apiSettings),
        ...
        results.dartCacheEntry = { fetchedAt: new Date().toISOString(), provider: 'openDart', schemaVersion: CACHE_SCHEMA_VERSION, payload: results.dart };
      }
    }
  ...
  if (apiSettings.globalProvider === 'yahooExperimental') {
    ...
    try {
      ...
      const entry = { fetchedAt: new Date().toISOString(), provider: 'yahooExperimental', schemaVersion: CACHE_SCHEMA_VERSION, payload };
...
async function fetchSecFinancialHistory(stock) {
  ...
  const data = await fetchJsonWithDiagnostics('SEC companyfacts', url, { allowErrorBody: true });
    ...
    for (const fsDiv of ['CFS', 'OFS']) {
      for (const endpoint of ['fnlttSinglAcntAll', 'fnlttSinglAcnt']) {
        try {
          ...
          const data = await fetchJsonWithDiagnostics(`OpenDART history ${endpoint}/${fsDiv}/${year}`, buildOpenDartApiUrl(endpoint, params), { openDartOkStatuses: ['000'] });
...
async function fetchSecTickerMap() {
  if (!secTickerMapPromise) {
    secTickerMapPromise = (async () => {
      const data = await fetchJsonWithDiagnostics('SEC ticker map', buildSecApiUrl('files/company_tickers.json'), { allowErrorBody: true });
      const map = {};
...
async function fetchSecFilings(stock, sinceISO) {
  ...
  if (!Number.isFinite(since.getTime())) throw new Error('잘못된 since 값');
  const company = await resolveSecCompany(stock);
  const data = await fetchJsonWithDiagnostics('SEC submissions', buildSecApiUrl(`submissions/CIK${company.cik}.json`), { allowErrorBody: true });
