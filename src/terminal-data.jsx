...
async function fetchYahooTimeSeries(symbol) {
  const types = [
    ...
    'annualLongTermDebt', 'annualCurrentDebt',
    'annualOperatingCashFlow', 'annualCapitalExpenditure',
  ].join(',');
  const period2 = Math.floor(Date.now() / 1000);
  const params = new URLSearchParams({ type: types, period1: '0', period2: String(period2) });
  const url = isProxiedOrigin()
    ? `/api/yahoo/timeseries/${encodeURIComponent(symbol)}?${params}`
    : `https://query1.finance.yahoo.com/ws/fundamentals-timeseries/v1/finance/timeseries/${encodeURIComponent(symbol)}?${params}`;
  try {
    const res = await fetch(url);
    if (!res.ok) return null;
    const data = await res.json();
    return data?.timeseries?.result ?? null;
  } catch { return null; }
}
...
function parseTimeSeriesStatements(results) {
  ...
  const typeMap = {
    ...
    annualCurrentDebt:        'shortLongTermDebt',
