...
function OverviewPanel({ stock }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 8, height: '100%' }}>
      ...
      <MetricsGrid metrics={stock.metrics} currency={stock.currency} activeDim={activeDim} asOf={stock.asOf}/>
    </div>
  );
}

// ─── Metrics grid ─────────────────────────────────────────────────────────────
function MetricsGrid({ metrics, currency, activeDim, asOf }) {
  const categories = [
    {
  ...
  const staleFlag = (() => {
    ...
    return Number.isFinite(parsed.getTime()) && (Date.now() - parsed.getTime()) > 30 * 86400000;
  })();
      ...
      <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 9 }}>
        {staleFlag && (
          <div style={{ fontSize: 9, color: T.yellow, background: `${T.yellow}15`, border: `1px solid ${T.yellow}44`, padding: '4px 8px' }}>
            ⚠ 데이터 갱신 30일+ 경과 ({asOf?.slice(0, 10)}) — REFRESH 권장
          </div>
        )}
        {categories.map(cat => {
