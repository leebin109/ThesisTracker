...
// ─── Metrics grid ─────────────────────────────────────────────────────────────
function MetricsGrid({ metrics, currency }) {
  const rows = [
    [
      ...
      <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {rows.map((row, ri) => (
          <div key={ri} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {row.map(({ key, label, fmt }) => {
              const v = metrics?.[key];
              const display = Number.isFinite(Number(v)) ? fmt(v) : '–';
              return (
                <div key={key} style={{ background: T.surface2, padding: '7px 10px', border: `1px solid ${T.borderSoft}` }}>
...
function ValuationPanel({ stock, onEdit }) {
  ...
  const mos = (price, v) => price ? `MOS ${(((price - v) / price) * 100).toFixed(1)}%` : '–';
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {scenarios.map(({ label, data, color }) => (
            <div key={label} style={{ background: T.surface2, border: `1px solid ${T.borderSoft}`, padding: '10px 12px' }}>
              <div style={{ fontSize: 9, color, letterSpacing: '0.14em', fontWeight: 700, marginBottom: 6 }}>{label}</div>
              ...
              {data?.mos > 0 && (
                <div style={{ fontSize: 10, color: T.cyan }}>{mos(data.price, cur)}</div>
              )}
            </div>
          ))}
        </div>
...
// ─── History / notes panel ────────────────────────────────────────────────────
function HistoryPanel({ stock, onAddNote }) {
  ...
  return (
    <Cell label="RESEARCH LOG" accent={T.cyan} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {notes.length === 0 && (
          <div style={{ color: T.inkFaint, fontSize: 11, paddingTop: 8 }}>기록 없음</div>
        )}
        {notes.map((n, i) => (
...
// ─── Review queue ─────────────────────────────────────────────────────────────
function ReviewQueue({ stocks, watchlistIds, onSelect }) {
  const items = useMemo(() => {
    return watchlistIds
      .map(id => stocks[id])
      .filter(Boolean)
      .map(s => ({ s, days: getDaysLeft(s.review?.next) }))
      .sort((a, b) => (a.days ?? Infinity) - (b.days ?? Infinity));
  }, [stocks, watchlistIds]);

  return (
    <Cell label="REVIEW QUEUE" accent={T.cyan} style={{ height: '100%' }}>
      <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {items.map(({ s, days }) => {
          const c = days === null ? T.inkFaint : days < 0 ? T.red : days <= 7 ? T.yellow : T.cyan;
          return (
            <div key={s.id} onClick={() => onSelect(s.id)}
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '7px 10px', background: T.surface2, cursor: 'pointer', border: `1px solid ${T.borderSoft}` }}>
              <span style={{ fontSize: 12, fontWeight: 600, color: T.ink }}>{s.symbol}</span>
              <span style={{ fontSize: 10.5, color: c, fontVariantNumeric: 'tabular-nums' }}>
                {days === null ? '미설정' : days < 0 ? `D+${Math.abs(days)} 지연` : days === 0 ? '오늘' : `D-${days}`}
              </span>
            </div>
          );
        })}
...
// ─── Peer comparison ─────────────────────────────────────────────────────────
function PeersPanel({ stock, stocks, watchlistIds, onSelect }) {
  ...
  const rows = [stock.id, ...peerIds].map(id => stocks[id]).filter(Boolean);
  const metrics = [
  ...
  return (
    <Cell label="PEER COMPARISON" accent={T.cyan} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '10px 14px', borderBottom: `1px solid ${T.borderSoft}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: 11, color: T.ink, fontWeight: 700 }}>{stock.symbol} vs watchlist peers</div>
          <div style={{ fontSize: 10, color: T.inkFaint, marginTop: 2 }}>Peer median 기준 premium/discount를 빠르게 확인합니다.</div>
        </div>
        <div style={{ fontSize: 10, color: T.cyan, fontVariantNumeric: 'tabular-nums' }}>{peerRows.length} peers</div>
      </div>
      <div style={{ flex: 1, overflow: 'auto', padding: 12 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10.5, fontVariantNumeric: 'tabular-nums' }}>
          <thead>
            <tr>
              ...
              {metrics.map(m => (
                <th key={m.key} style={{ textAlign: 'right', color: T.inkFaint, padding: '7px 8px', borderBottom: `1px solid ${T.border}` }}>{m.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            {peerRows.length > 0 && (
              <tr>
                ...
                <td style={{ color: T.inkFaint, padding: '8px', borderTop: `1px solid ${T.border}` }}>watchlist peers</td>
                {metrics.map(m => (
        ...
        {peerRows.length === 0 && (
          <div style={{ padding: 18, color: T.inkFaint, fontSize: 11, textAlign: 'center' }}>
            비교할 peer가 없습니다. 관심종목을 더 추가하면 자동으로 비교 대상이 됩니다.
          </div>
        )}
      </div>
    </Cell>
...
function SettingsDataPanel({
  ...
  const cacheCount = Object.keys(dataCache || {}).length;
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, height: '100%' }}>
      ...
      <Cell label="BACKUP / CACHE" accent={T.cyan} style={{ height: '100%' }}>
        <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: T.surface2, border: `1px solid ${T.borderSoft}`, padding: 12 }}>
            <div style={{ fontSize: 9, color: T.inkFaint, letterSpacing: '0.12em', marginBottom: 6 }}>CACHE</div>
            <div style={{ color: T.ink, fontSize: 13, marginBottom: 10 }}>{cacheCount} cached entries</div>
            <button onClick={onClearCache} style={{ ...btnSt, color: T.red, border: `1px solid ${T.red}` }}>CLEAR CACHE</button>
          </div>
      ...
      <Cell label="ALERT SETTINGS" accent={T.red} style={{ height: '100%', overflow: 'hidden' }}>
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 12, height: '100%', overflowY: 'auto', fontSize: 11 }}>
          ...
          <button onClick={onRefresh}
            style={{ ...btnSt, color: T.amber, border: `1px solid ${T.amber}`, marginTop: 4 }}>
            REFRESH NOW
          </button>
          {alertStatus && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 8, height: '100%' }}>
        ...
        <div style={{ padding: 12, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {groups.map(group => {
            ...
            return (
              <div key={group}>
                <div style={{ fontSize: 9, color: T.inkFaint, letterSpacing: '0.14em', marginBottom: 6 }}>{group}</div>
  ...
  return (
    <Cell label="CALENDAR" accent={T.cyan} style={{ height: '100%', overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 8, height: '100%' }}>
        ...
        <div style={{ padding: 12, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 7 }}>
          {events.length === 0 && <div style={{ color: T.inkFaint, fontSize: 12 }}>No calendar events yet.</div>}
          {events.map(e => {
            const overdue = e.date && e.date < today && !e.done;
            return (
              <div key={`${e.stockId}-${e.id}`} style={{ display: 'grid', gridTemplateColumns: '96px 76px 1fr auto', gap: 10, alignItems: 'center', padding: '8px 10px', background: T.surface2, border: `1px solid ${overdue ? T.red + '55' : T.borderSoft}`, opacity: e.done ? 0.55 : 1 }}>
                <div style={{ fontSize: 10, color: overdue ? T.red : T.inkFaint }}>{e.date || '--'}</div>
...
function App() {
    ...
    getSb().from('user_data').select('payload').eq('user_id', session.user.id).single()
      .then(({ data, error }) => {
        if (error || !data?.payload) {
          // First login — push local state to Supabase
          const snap = { stocks, watchlistIds, activeId, apiSettings, dataCache, dartCorpMap, alerts, alertSettings, portfolio };
        ...
        if (p.apiSettings && typeof p.apiSettings === 'object') setApiSettings(s => ({ ...s, ...p.apiSettings }));
        if (p.dataCache && typeof p.dataCache === 'object') setDataCache(p.dataCache);
  ...
  }, [session, stocks, watchlistIds, activeId, apiSettings, dataCache, dartCorpMap, alerts, alertSettings, portfolio]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Toast helpers ──────────────────────────────────────────────────────────
  const toast = useCallback((msg, kind = 'info', ms = 4000) => {
  ...
  const panelContent = {
    ...
    F4: (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, height: '100%' }}>
        <HistoryPanel stock={stock} onAddNote={handleAddResearchNote}/>
        <ReviewQueue stocks={stocks} watchlistIds={watchlistIds} onSelect={setActiveId}/>
      </div>
    ),
    F5: (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 8, height: '100%' }}>
        <ChartPanel stock={stock} onRefresh={handleRefresh} refreshing={refreshing} fetchStatus={fetchStatus}/>
      </div>
    ),
    F6: (
    ...
    F7: (
      <PeersPanel stock={stock} stocks={stocks} watchlistIds={watchlistIds} onSelect={setActiveId}/>
    ),
    F8: (
